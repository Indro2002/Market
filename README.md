  ...
  <div class="card admin">
    ...
    <div class="row" style="margin-top:8px">
      ...
      <input
        ...
        min="0"
        step=".01"
        placeholder="Çmimi / Çmimi për kg"
      >
    ...
    <p class="small">
      Për produktet me kg, çmimi vendoset për 1 kg.
    </p>
  </div>
...
<section id="sale">
  ...
  <div class="card">
    <div class="row">
      <input
        id="saleScan"
        class="col"
        placeholder="Emër ose barkod / Skaner USB"
        autocomplete="off"
      >
    ...
    <button style="margin-top:10px" onclick="manualAdd()">
      ➕ Shto në shportë
    </button>

    <p class="small">
      Për produktet me kg vendos peshën para shtimit.
    </p>
  </div>
  ...
  <div class="card">
    ...
    <div class="loyalty">
      ...
      <div id="loyaltyInfo" class="small">
...
<section id="bil">
  <div class="card">
    ...
    <p class="small">
      Thyerja fillestare përdoret për kusur dhe nuk është shitje.
    </p>
    ...
    <div class="row">
      ...
      <div class="col">
        ...
        <div style="padding:10px 0">
          <b><span id="activeFloat">0.00</span> ALL</b>
        </div>
      </div>
    </div>

    <button style="margin-top:10px" onclick="openKasa()">
      Hap Kasa
    </button>
  </div>
  ...
  <div class="card">
    ...
    <div class="stats">
      <div class="stat">
        <div class="stat-label">Cash ALL Neto</div>
        <div class="stat-value">
          <span id="cashBalanceALL">0.00</span> ALL
        </div>
      </div>
      ...
      <div class="stat">
        ...
        <div class="stat-value">
          <span id="cardBalanceALL">0.00</span> ALL
        </div>
      </div>

      <div class="stat">
        <div class="stat-label">Cash EUR</div>
        <div class="stat-value">
          <span id="cashBalanceEUR">0.00</span> EUR
        </div>
      </div>

      <div class="stat">
        <div class="stat-label">Thyerja</div>
        <div class="stat-value">
          <span id="activeFloatCard">0.00</span> ALL
        </div>
      </div>
    </div>
    ...
    <p>
      Cash ALL Neto:
      <b><span id="daily">0.00</span> ALL</b>
    </p>

    <p>
      Kartë:
      <b><span id="dailyCard">0.00</span> ALL</b>
    </p>

    <p>
      Cash EUR:
      <b><span id="dailyEUR">0.00</span> EUR</b>
    </p>
    ...
    <p class="small">
      Pagesa EUR fut eurot në Cash EUR dhe kusuri në ALL
    ...
    <button
      class="secondary"
      onclick="exportDailyClosuresExcel()"
    >
      📊 Eksporto Bilancet Ditore
    </button>
  </div>
</section>
...
<section id="settings">
  <div class="card admin">
    ...
    <button style="margin-top:10px" onclick="saveSettings()">
      Ruaj Cilësimet
    </button>
  </div>
</section>
...
let exchangeRate=Math.max(
  .0001,
  num(localStorage.getItem('exchangeRate'),100)
);

let reorderConfig=parse(
  localStorage.getItem('reorderCfg'),
  {
    leadDays:3,
    reviewPeriodDays:7,
...
function productData(product){
  return {
    ...
    minStock:Math.max(
      0,
      num(product.minStock??product.min)
    )
  };
}
...
Object.values(loyaltyCustomers).forEach(customer=>{
  customer.points=Math.max(
    0,
    num(customer.points)
  );

  customer.totalEarned=Math.max(
    customer.points,
    num(customer.totalEarned,customer.points)
  );

  customer.totalUsed=Math.max(
    0,
    num(customer.totalUsed)
  );

  customer.totalSpentALL=Math.max(
    0,
    num(customer.totalSpentALL)
  );

  customer.transactions=Math.max(
    0,
    num(customer.transactions)
  );
});
...
function renderProducts(){
  ...
  products.forEach((product,index)=>{
    ...
    if(product.expiry){
      ...
      if(expiry<today){
        ...
        if(days<=7){
          rowClass='expiring';
          alarm=
            `<span class="warn">Skadon ${days}d</span>`;
        }
      }
    }
    ...
    if(product.s<=product.minStock){
      ...
      alarm+=(alarm?' • ':'')+
        '<span class="red">Stok i ulët</span>';
    }

    $('prodTable').insertAdjacentHTML('beforeend',`
      <tr class="${rowClass}">
        <td>
          <input value="${esc(product.n)}"
            onchange="updateProduct(${index},'n',this.value)">
        </td>

        <td>
          <input value="${esc(product.b)}"
            onchange="updateProduct(${index},'b',this.value)">
        </td>
        ...
        <td>
          <input type="number" min="0" step=".01"
            value="${product.p}"
            onchange="updateProduct(${index},'p',this.value)">
        </td>

        <td>
          <input type="number" min="0" step=".001"
            value="${product.s}"
            onchange="updateProduct(${index},'s',this.value)">
        </td>
  ...
  products.forEach(product=>{
    ...
    if(product.expiry){
      ...
      if(expiry<now){
        ...
        result.push(
          `Skadon së shpejti: ${product.n} (${days} ditë)`
        );
      }
    }
  });
...
function refreshLoyaltyUI(){
  ...
  $('loyaltyInfo').innerHTML=`
    ...
    Pikë aktuale:
    <span class="green">${customer.points}</span><br>
    Pikë totale:
    <b>${customer.totalEarned}</b><br>
    Pikë të përdorura:
    <b>${customer.totalUsed}</b><br>
    Shpenzuar:
    <b>${money(customer.totalSpentALL)} ALL</b>
  `;

  $('useLoyaltyPoints').disabled=
    customer.points<=0;

  if(customer.points<=0){
    $('useLoyaltyPoints').checked=false;
    $('loyaltyDiscount').textContent=
      'Klienti nuk ka pikë për përdorim.';
  }else{
    $('loyaltyDiscount').textContent=
      `Mund të përdorë deri në ${
        money(customer.points*ALL_PER_POINT)
      } ALL zbritje.`;
  }
...
function updateLoyaltyAfterPayment(record){
  ...
  const earned=earnedPoints(record.amountPaid);
  ...
  customer.totalUsed=
    num(customer.totalUsed)+used;

  customer.totalSpentALL+=
    num(record.amountPaid);
...
function addToCart(product){
  ...
  if(quantity>product.s){
    alert('Pesha/sasia tejkalon stokun.');
    return;
  }
...
function updateBalancesAfterPayment(record){
  /*
    Cash ALL:
    - Cash ALL: shton amountPaid.
    - Cash EUR: heq kusurin në ALL.
    - Kartë: nuk prek Cash ALL.

    Cash EUR:
    - Shton vetëm eurot që klienti dha.

    Kartë:
    - Shton vetëm shumën e kartës.
  */
...
function pay(){
  ...
  const report=buildInvoiceHtml(record);
...
function buildInvoiceHtml(record){
  const cashLabel=record.paymentMethod==='cash_eur'
    ? `${money(record.cashGivenRaw)} EUR`
    : `${money(record.cashGivenRaw)} ALL`;

  const equivalentLabel=record.paymentMethod==='cash_eur'
    ? `${money(record.cashGivenALL)} ALL`
    : `${money(record.cashGivenALL)} ALL`;

  const changeLabel=record.paymentMethod==='card'
    ? 'Nuk aplikohet'
  ...
  const lines=(record.items||[]).map(item=>`
    <tr>
      <td>${esc(item.n)}</td>
      <td>${lineQuantity(item)}</td>
      <td>${linePrice(item)}</td>
      <td>${money(item.q*item.p)} ALL</td>
    </tr>
  `).join('');
  ...
  if(record.client){
    html+=`
      ...
      <div class="invoice-summary">
        <div>
          Pikë të fituara në këtë blerje:
          <b>${record.loyaltyPointsEarned}</b>
        </div>

        <div>
          Pikë totale të mbledhura:
          <b>${record.customerTotalEarned}</b>
        </div>

        <div>
          Pikë të përdorura:
          <b>${record.loyaltyPointsUsed}</b>
        </div>

        <div>
          Pikë të disponueshme:
          <b>${record.loyaltyBalanceAfter}</b>
        </div>
      </div>
    `;
  }
...
function printInvoice(record){
  const invoiceHtml=buildInvoiceHtml(record);
  ...
  if(!printWindow){
    alert(
      'Lejo popup-et në browser për të printuar faturën.'
    );
    return;
  }
    ...
    $('histTable').insertAdjacentHTML('beforeend',`
      <tr>
        ...
        <td>
          ...
          <button class="danger" onclick="deleteSale(${index})">
            🗑️
          </button>
        </td>
      </tr>
...
function viewSale(index){
  ...
  $('viewBody').innerHTML=buildInvoiceHtml(record);
...
window.exportDailyClosuresExcel=
  exportDailyClosuresExcel;
